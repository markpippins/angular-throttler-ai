...
<aside class="w-80 h-full flex flex-col bg-[rgb(var(--color-surface-sidebar))] border-l border-[rgb(var(--color-border-base))] text-sm animate-slide-in-right">
  @if (item(); as currentItem) {
    ...
    <div class="p-4 space-y-3 overflow-y-auto flex-1">
      @if (isImageFile() && currentItem.content) {
        <div class="mb-4">
            <h4 class="font-semibold text-[rgb(var(--color-text-base))] mb-2">Preview</h4>
            <div class="flex items-center justify-center p-2 border border-[rgb(var(--color-border-base))] rounded-md bg-[rgb(var(--color-background))]">
              <img [src]="currentItem.content" [alt]="currentItem.name" class="max-w-full max-h-48 object-contain rounded">
            </div>
        </div>
      }
